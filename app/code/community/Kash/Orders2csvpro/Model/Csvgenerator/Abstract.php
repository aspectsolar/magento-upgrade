...
// 			fputcsv($fp, $headerTitles, $fileStructur->getDelimiter(), $fileStructur->getEnclosure());
	...
	protected function writeLines($element, $elementItems, $order, $fileStructur, $fp = null)
	{
		...
		foreach ($columns as $column){
			...
			if($value == 'ProjectId'){
				if(strtolower(trim($billingAddress->getName())) == strtolower(trim('QuiBids Holdings LLC'))){
					$value = '60';
				}else{
					$value = '59';
				}
			...
			if($value == 'FedEx - Ground'){
				$value = 'FedEx';
			}
		
			if($value == 'fedex_FEDEX_GROUND'){
				$value = 'Ground';
			}
	...
	public function transformValues($values, $columns){
		...
		$pieceCode = array();
		 ...
		 foreach ($collection as $product) {
			 ...
			 $pieceCode[] = $product->getData('export_file_sku');
		 }		
		...
		$skip_sku = array('powerpackagepro1');
		...
		foreach ($columns as $column){
				if($count==63){
					if(in_array($values[$column->getValue()],$model)){
						...
						if(in_array($values[$column->getValue()],$skip_sku)){
							$rowItems[] = '';
							$flag = 1;
						}else{
