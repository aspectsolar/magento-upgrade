                    ...
                    if ($this->getGroupByType($optionType)==self::OPTION_GROUP_SELECT) {
                        if (count($option->getValues())>0) {
                            foreach ($option->getValues() as $value) {
                                ...
                                list($price, $priceType, $isProductPrice, $taxClassId, $oldPrice) = $helper->getOptionPriceAndPriceType($value->getPrice(), $value->getPriceType(), $value->getSku(), $product->getStore());
                                if ($isProductPrice) {
                        ...
                        list($price, $priceType, $isProductPrice, $taxClassId, $oldPrice) = $helper->getOptionPriceAndPriceType($option->getPrice(), $option->getPriceType(), $option->getSku(), $product->getStore());
                        if ($isProductPrice) {
                            if ($helper->isSpecialPriceEnabled()) {
                                if ($oldPrice) {
