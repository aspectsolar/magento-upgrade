...
class MageWorx_CustomOptions_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    // return $price, $priceType, $flag => true - use product price, false = no; $taxClassId, $oldPrice
    public function getOptionPriceAndPriceType($price, $priceType, $sku, $store) {
        if ($sku!='' && $this->isSkuPriceLinkingEnabled()) {
            if (isset($this->_products[$sku]['price'])) {
                return array($this->_products[$sku]['price'], 'fixed', true, $this->_products[$sku]['tax_class_id'], $this->_products[$sku]['old_price']);                
            } else {
        ...
        return array($price, $priceType, false, 0, 0);
    }    
    ...
    public function getOptionTierPriceAndType($optionValue, $optionQty, $store) { 
        ...
        if ($this->isSpecialPriceEnabled() && $optionValue->getSpecialPrice()>0) {
            ...
            list($optionPrice, $optionPriceType, $isProductPrice, $taxClassId) = $this->getOptionPriceAndPriceType($optionValue->getPrice(), $optionValue->getPriceType(), $optionValue->getSku(), $store);
        }
        ...
        return array($optionPrice, $optionPriceType, $isProductPrice, $taxClassId);
    }
    ...
    public function _getOptionConfiguration($option, $value = null) {
        ...
        // price
        if ($value && $this->isSpecialPriceEnabled() && $value->getSpecialPrice()>0) {
